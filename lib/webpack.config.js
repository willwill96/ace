...
console.log(path.resolve(__dirname, path.join(cesiumSource, cesiumWorkers)))
const gitEnv = () => {
...
const base = ({
  ...
  plugins: [
    ...
    new CopywebpackPlugin([
      {
        from: path.resolve(__dirname, path.join(cesiumSource, cesiumWorkers)),
        to: 'Workers',
      },
    ]),
    new CopywebpackPlugin([
      {
        from: path.resolve(__dirname, path.join(cesiumSource, 'Assets')),
        to: 'Assets',
      },
    ]),
    new CopywebpackPlugin([
      {
        from: path.resolve(__dirname, path.join(cesiumSource, 'Widgets')),
        to: 'Widgets',
      },
    ]),
    new webpack.DefinePlugin({
      // Define relative base path in cesium for loading assets
      CESIUM_BASE_URL: JSON.stringify('/search/catalog'),
    }),
  ],
  externals: {
    'react/addons': true,
